...
def load_model():
    ...
    model = timm.create_model(
        "timm/vit_small_patch16_dinov3.lvd1689m", pretrained=True, num_classes=0
    )
    model.eval()  # 设置为评估模式
    # 冻结模型的所有参数
...
def export_onnx(custom_model, data_config, onnx_path="dino_classifier.onnx", opset=18):
    # 导出 ONNX，设置动态 batch 维度，方便 ONNX Runtime/TensorRT 使用
    custom_model.eval().to("cpu")
    ...
    print(f"ONNX 导出完成: {onnx_path} (opset={opset})")
...
if __name__ == "__main__":
    ...
    export_onnx(custom_model, data_config, opset=18)
